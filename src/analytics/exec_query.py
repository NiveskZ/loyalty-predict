...
def exec_query(table, db_origin, db_target, dt_start, dt_stop, monthly, mode='append'):
    ...
    for i in tqdm(dates):
        with engine_analytical.connect() as conn:
            try:
                conn.execute(sqlalchemy.text(f"DELETE FROM {table} WHERE dtRef = date('{i}', '-1 day') OR dtRef IS null"))
                conn.commit()
...
def main():
    ...
    parser.add_argument("--table", type=str, help="Tabela que será processada com mesmo nome do arquivo.")
