...
tb_hora_cliente AS (
    ...
    SELECT IdCliente,
            ...
            sum(CASE WHEN dtDia >= date('2026-02-01', '-7 day') THEN duracao ELSE 0 END) AS qtdeHorasD7,
            sum(CASE WHEN dtDia >= date('2026-02-01', '-14 day') THEN duracao ELSE 0 END) AS qtdeHorasD14,
            sum(CASE WHEN dtDia >= date('2026-02-01', '-28 day') THEN duracao ELSE 0 END) AS qtdeHorasD28,
            sum(CASE WHEN dtDia >= date('2026-02-01', '-56 day') THEN duracao ELSE 0 END) AS qtdeHorasD56
...
tb_intervalo_dias AS (
    ...
    SELECT *,
            ...
            avg(CASE WHEN dtDia >= date('2026-02-01','-28 day') THEN julianday(dtDia) - julianday(lagDia) END) AS avgIntervaloDiasD28
    FROM tb_lag_dia
